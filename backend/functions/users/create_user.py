...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        Validator.validate_required_fields(body, ["email", "given_name", "family_name", "role"])
        ...
        try:
            ...
            logger.info("User created in Cognito", user_sub=user_sub, email=email, role=role)
            ...
            if error_code == "UsernameExistsException":
                ...
                raise SavingGraceError(
                    message=f"Failed to create user in Cognito: {str(e)}", status_code=500
                )
        ...
        try:
            ...
            logger.info("User added to Cognito group", user_sub=user_sub, group=role)
            ...
            try:
                ...
                logger.error(
                    "Failed to cleanup user after group assignment failure", error=cleanup_error
                )

            raise SavingGraceError(
                message=f"Failed to assign user to group: {str(e)}", status_code=500
            )
        ...
        try:
            ...
            logger.info("User profile saved to DynamoDB", user_id=user_sub)
            ...
            raise SavingGraceError(
                message=f"Failed to save user profile: {str(e)}", status_code=500
            )
