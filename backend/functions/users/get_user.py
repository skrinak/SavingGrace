...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            logger.info(
                "User profile retrieved from DynamoDB",
                user_id=user_id
            )
        ...
        try:
            cognito_response = cognito_client.admin_get_user(
                ...
                Username=user_profile.get("email")
            )
            ...
            if 'UserLastModifiedDate' in cognito_response:
                cognito_user_data['last_modified'] = cognito_response['UserLastModifiedDate'].isoformat()

            logger.info(
                "User details retrieved from Cognito",
                user_id=user_id
            )
            ...
            if error_code == 'UserNotFoundException':
                ...
                raise SavingGraceError(
                    message=f"Failed to retrieve user from Cognito: {str(e)}",
                    status_code=500
                )
